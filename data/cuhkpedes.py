...
class CUHKPEDESDataset(data.Dataset):
    ...
    #BERT
    pklname_list = ['./datasets/processed/BERT_id_train_64_new.npz', './datasets/processed/BERT_id_val_64_new.npz',
                    './datasets/processed/BERT_id_test_64_new.npz']
    ...
    def __init__(self, root, split, max_length, transform=None, target_transform=None,
        ...
        if self.split == 'train':
            ...
            with open(self.pklname, 'rb') as f_pkl:
...
class CUHKPEDESDataset_(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        caption = torch.tensor(caption).long()
        attention_mask = torch.tensor(attention_mask).long()
        return img_path, caption, label, attention_mask
...
if __name__ == '__main__':
    ...
    data_split = CUHKPEDESDataset(args.dir, split, args.max_length,transform=transform)
